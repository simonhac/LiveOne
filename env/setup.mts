...
// `--yes`: in a fresh worktree tsx isn't installed yet, so plain `npx tsx` would hang on
// an install prompt (see LiveOne#355). `.mts` forces ESM (works in non-"type":module repos).
