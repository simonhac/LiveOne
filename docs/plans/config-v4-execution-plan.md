...
| 5 — v4 dashboard doc model + dual renderer | 🔨 IN PROGRESS | **pure core landed** (`lib/dashboard/v4.ts` types, `card-types.ts`, `v4-validate.ts` zod validator + `normalizeDocV4` + `collectRefs`, `v3-to-v4.ts` rewriter; adversarially reviewed — dup-id + config-less-chart fixes applied) + **`resolve-shell.ts`** (pure `resolveNodeContexts` §8.1 inheritance + `resolveDashboardShell` behind an injected `ShellResolver` → deduped data handles for SSR; 24 tests total; zod added). **Next in-phase:** merged `CARD_RENDERERS` + recursive `<NodeView>` (reuse existing card/tile plugins), then the dual-shape (v3+v4) SSR render window. |
