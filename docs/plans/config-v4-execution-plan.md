...
## ▶ NEXT ACTION — Phase 8 IN PROGRESS (P0 + 0035 + #242 landed; Group A verification underway → Group B build → Group C window)
...
>
> **The remaining Phase-8 work is three groups** (detail in the plan of record): **Group A** — dark,
> mergeable prep that finishes the transform + verification (single-source stage-2 → `registry-populate.ts`;
> owner-token auto-create + grant reshape in `config-transform.ts`; `authz-check.ts` + DDL/content parity
> checks; docs) — **IN PROGRESS**. **Group B** — the all-or-nothing cutover BUILD (DAO rid-flip,
> `systems`→`devices` rename, delete virtual-system synthesis, `?systemId=` alias, KV move, drop
> `user_systems`, unify `sharing.ts`/narrow `grants.ts`, DAO-equivalence sweep) — deploys only in-window.
> **Group C** — schedule + run the window (dev first, prod next day).
>
> **Done so far:**
...
> - **Group A (this branch)** — single-sourced the additive stage-2 population into `registry-populate.ts`
...
margin = 15 min ≤ the 30-min pause target ≪ the 2h budget); **parity 23/23 green** — per-column content
checksums matching old-vs-twin across all ~21M hot-table rows (15.5M `point_readings` + 6M `agg_5m`), plus
registries, bindings→`pt_` uuid, derivations (run-detector + HWS), the dashboards int→uuid PK swap
...
| 7 — cutover rehearsal harness                          | ✅ DONE (PR #237)             | transform + parity/window harness validated on prod-restore PS-5 branches: T_window ~5min → single-window GO; parity 23/23 (per-column checksums across ~21M rows + full stage-5 config transform incl. dashboards int→uuid PK swap + HWS). Run log → `config-v4-phase7-rehearsal-harness.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 8 — THE CUTOVER                                        | 🔄 IN PROGRESS               | #242 (cron gate + C7 mirror + registry-sync) + Group A (transform completion + authz/DDL verification + docs) landed dark; Group B = cutover build; Group C = window. See [config-v4-phase8-cutover.md](config-v4-phase8-cutover.md). Single windowed op; pauses materialization, not pollers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
