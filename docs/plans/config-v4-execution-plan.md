...
| 6 — `/api/v4/*` route surface | 🔨 IN PROGRESS | **dashboard doc routes landed** (dark): `GET`/`PUT` `app/api/v4/dashboards/[id]` + `POST …/validate`; whole-doc `PUT` = `validateDocV4`→422, §8.4 area-readability→403, `updateDashboardDoc` (SELECT FOR UPDATE + `If-Match`→412 + revision bump) → 200 + ETag. Addressed by serial id (db_ TypeID at cutover); `dashboard_revisions` history deferred (uuid-keyed). PUT path verified on `liveone-dev` (rolled back: rev 1→2, jsonb `doc`). **Remaining:** the rest of §9.2 (devices/areas/bindings/derivations/shares/grants/export-import) — mostly cutover-era. |
