...
1. ✅ **Done (this branch, TypeID-native):** read-only `GET /api/v4/areas` (list), `/areas/{id}` (detail:
   ...
   /`findReadableArea` (`lib/areas/http.ts`); seed builders `lib/dashboard/v4-seed.ts`. Verified against real
   dev data (HTTP curl blocked — the dev server was globally 500ing, unrelated; the route logic was exercised
...
throwaway prod-snapshot branch, iterated until all checks pass AND the 13M/3M rewrite fits the window. Now
unblocked — Phases 1–6 are live and #233 is merged + deployed. A **§15 follow-up** (make `oe-grid` area-level
...
> (this branch, TypeID-native `ar_`/`pt_`): read-only `GET /api/v4/areas`, `/areas/{id}`,
