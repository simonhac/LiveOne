...
| 5 — v4 dashboard doc model + dual renderer | 🔨 IN PROGRESS | **pure core landed** (`lib/dashboard/v4.ts` types, `card-types.ts`, `v4-validate.ts` zod validator + `normalizeDocV4` + `collectRefs`, `v3-to-v4.ts` rewriter; 21 tests; zod added; adversarially reviewed — dup-id + config-less-chart fixes applied). **Next in-phase:** merged `CARD_RENDERERS`/recursive `<NodeView>`, `resolve-shell.ts`, dual-shape (v3+v4) render window. |
