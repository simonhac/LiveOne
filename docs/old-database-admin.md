...
This document records the old `/admin/readings` database admin surface that was stripped back after the legacy SQLite store was decommissioned. Use it as a map for rebuilding a smaller Postgres-native tool, not as a target to restore wholesale.

## What It Did

The page was a catch-all operations panel for the legacy SQLite-era database:
...
- In development, opened a sync modal that previewed and streamed a production-to-local SQLite sync.
...
- `app/api/admin/storage/route.ts` - old SQLite storage stats response and `force-reload-caches` POST action.
...
- Treat dev seeding as a new Postgres-to-dev workflow; do not reuse the legacy SQLite sync route shape without rethinking safety and data scope.
