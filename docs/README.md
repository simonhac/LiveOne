...
- [plans/sigenergy-energy-backfill-cadence.md](plans/sigenergy-energy-backfill-cadence.md) — **Sigenergy interval energy lags 7–14 days** (the Kew/Kutis "no data since 14 Jul" report). Not data loss and not the config-v4 cutover: Sigenergy is the only vendor with no live-poll path for energy, so the weekly `repair-coverage` backstop (`graceDays: 7`) is doing a primary path's job while the purpose-built `/api/cron/sigenergy-backfill` sits unscheduled. Fix = schedule it daily + revisit the per-vendor grace
