...
- [architecture/data-model.md](architecture/data-model.md) — data semantics & invariants; schema source of truth is `lib/db/planetscale/schema.ts`. Holds the config-v4 invariants: the uuid/rid **seam rule**, the TypeID scheme, eager areas, fixed-offset days, the v4 dashboard document + its security invariant, the **permanent-shim list**, and how Postgres errors arrive through PlanetScale
...
- [plans/config-v4-clean-sheet.md](plans/config-v4-clean-sheet.md) — **the canonical design doc** and the _why_ behind the shipped model: one TypeID space (integer handle retired), `systems`→`devices`, eager areas (tz/location area-only, fixed-offset days endorsed), per-role bindings with deterministic slot resolution, trackers generalized to `derivations`, recursive group/card dashboard docs (tile=small card) with whole-doc PUT + revisions, one share-token semantics. **Supersedes** info-producers-consumers + identity-address-split-and-labels and parts of areas-and-dashboards. The invariants it locked now live in [architecture/data-model.md](architecture/data-model.md)
