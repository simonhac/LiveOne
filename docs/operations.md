...
| -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `liveone DB backup — …` daily row (✅/❌/🖐️/⬜ ticks) + `🔴 *liveone* backup FAILED …` | [the-gitfather](https://github.com/simonhac/the-gitfather) `pg-backup.yml` via `.github/workflows/pg-backup.yml` (2-hourly at :17 UTC; profile `pg-backup/liveone.env`) | Off-site `pg_dump -Fc` → R2, GFS 2hourly→daily/weekly/monthly (lifecycle 2d / 21d / 70d / 400d; the 16:17 UTC run promotes). One Slack message per day, updated in place; a failure also posts to `OBSERVATIONS_ALERT_WEBHOOK_URL`. 🔴 = the dump or upload failed → check the Action log; confirm PITR is still covering the gap.                                                                                                                              |
| `✅/🔴 PG restore-drill (liveone) …`                                                   | the-gitfather `pg-restore-drill.yml` via `.github/workflows/pg-restore-drill.yml` (Mondays 17:37 UTC ≈ Tue 03:37 Sydney)                                                | Restores the newest matching dump into a throwaway target and asserts `point_readings ≥ MIN_RATIO × live` (default 0.95). 🔴 = backups aren't restoring cleanly → **investigate before trusting the backups**; re-run the drill. (`scripts/utils/restore-drill-pg.sh` is retained locally as the manual restore / `liveone-dev` seed helper.)                                                                                                                   |
