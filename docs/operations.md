...
## Nightly coverage repair

**`app/api/cron/repair-coverage/route.ts`** — a Vercel cron (nightly, `vercel.json`) that finds
coverage gaps in the re-fetchable vendors (Amber/OpenElectricity/Sigenergy) and backfills them. Most
nights it scans a shallow window (~10 days); on Mondays UTC it sweeps each provider's full 90. Unlike
the monitor above it **posts every run** (a nightly heartbeat), not only on failure. The message is an
itemised report — `🟢 ok` (nothing to do, or all repaired) / `🟡 warn` (some gap-days `unsettled` —
the vendor API had no data yet, retried next run — or `deferred (cap)` / not-yet-landed) / `🔴 alert`
...
| -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `liveone DB backup — …` daily row (✅/❌/🖐️/⬜ ticks) + `🔴 *liveone* backup FAILED …` | [the-gitfather](https://github.com/simonhac/the-gitfather) `pg-backup.yml` via `.github/workflows/pg-backup.yml` (2-hourly at :17 UTC; profile `pg-backup/liveone.yaml`) | Off-site `pg_dump -Fc` → R2, GFS 2hourly→daily/weekly/monthly (lifecycle 2d / 21d / 70d / 400d; the 16:17 UTC run promotes). One Slack message per day, updated in place; a failure also posts to `OBSERVATIONS_ALERT_WEBHOOK_URL`. 🔴 = the dump or upload failed → check the Action log; confirm PITR is still covering the gap.                                                                                                                                                                                           |
| `✅/🔴 PG durable-verify (liveone) …`                                                  | the-gitfather `pg-durable-verify.yml` via `.github/workflows/pg-durable-verify.yml` (daily 18:37 UTC ≈ 04:37 Sydney)                                                     | Hash-checks **every** durable `.dump` object against the SHA-256 recorded at backup time, full-restores the freshest `daily`, and re-restores the newest weekly/monthly past `verify-durable.retest-days`; asserts `point_readings ≥ min-row-ratio × live` (0.95). Filters by `.dump`, so legacy `.dump.gz` is skipped. 🔴 = a backup failed its hash or restore → **investigate before trusting the backups**. (`scripts/utils/restore-drill-pg.sh` is retained locally as the manual restore / `liveone-dev` seed helper.) |
| `🔴 PG backup STALE (liveone) …` / `🟡 … triggered a catch-up backup`                  | the-gitfather `pg-staleness-check.yml` via `.github/workflows/pg-staleness-check.yml` (hourly at :20)                                                                    | Asserts a fresh `2hourly/` object landed (< `STALE_HOURS`=5h); self-heals a _missed_ tick by re-triggering `pg-backup.yml`. 🟡 = missed tick, catch-up triggered (informational). 🔴 = stale **and** not auto-retrying (last backup looks broken, not missed) → investigate.                                                                                                                                                                                                                                                 |
