...
5. **Units take the value's colour, and word units are caps** (`3.2KW`, `16.3KWH`, `$84/MWH`) —
   the Activity app's `3.20KM`. Glyph units (`%`, `°C`, `¢`, `$`) are untouched. Done in CSS on
   `<Value>`'s `data-unit` spans inside `.tile-scope`, so every nested value follows without being
   told; outside a tile, units stay muted and cased as written
   ([number-typography.md](number-typography.md)).
   ⚠️ Caps erase `mW`/`MW` on screen — acceptable only because no tile shows a milli-unit.
6. **One hero per tile**, glanceable in under two seconds: hero plus at most two supporting facts in
   a small tile. A **medium** tile (`TilePlugin.span: 2`) may carry a viz _and_ a stat column.
7. **Rings are fat.** `ProgressRing` defaults to a 0.14 stroke ratio, round caps, a track of the
   same hue at 25%, an optional gradient, a glyph riding the arc's tip (turned to its tangent) and an
   optional notch for a target. `ConcentricRings` is the Activity Rings set.
8. **Mini charts are bars on hairlines** (`MiniBars`): thin round-capped bars in the series colour,
...
9. **Direction is a chip** (`DirectionChip`): a coloured chevron in a grey disc. **Up** = energy
...
10. **Stale is quiet.** No diagonal hatch, no dimmed box, and never a grey-out: LIVE values (a hero
...
11. **Motion is small.** Rings and bars ease 400ms (`.tile-ease`); nothing moves under
    `prefers-reduced-motion`.
12. **Brand marks are guests.** The Amber and Tesla marks sit in the title slot. Amber's price disc
