...
21. **Device → 0..1 area, Home Assistant's shape** (14 September 2026, migrations 0070–0074): membership collapses from the many-to-many `area_members` onto one nullable column, `devices.area_id`. The generality it removed was never authored by a human — it existed so a device could sit in both its eagerly-minted "area of one" and its real site — and it had cost two production defects: every Kutis EV charge priced at $0.00 for two months because something had to GUESS which of a device's areas priced it, and a flow-eligibility dedupe whose only job was stopping a child area claiming its parent's Sankey. `day_offset_min` moves onto the device (a bucketing key can't be coarser than the table it buckets); `display_timezone`/`location` stay on the area behind `area → owner → platform` resolution, so an **ambient** device — the ownerless OpenElectricity NEM regions, HA's `entry_type=SERVICE` — is fully functional in no area at all. 0072 dropped `primary_area_id`'s NOT NULL, which was the only thing still forcing the mint; 0073 added `users.default_area_id` so onboarding has somewhere to put a device that is not a private shell; 0074 dropped `area_members` and the column. Existing areas-of-one are kept forever — they key uuid-addressed history.
...
7. **Observations ingest stall** (9 September 2026): a poison Amber backfill head-of-line blocked the strictly-FIFO queue, stopping all ingest for ~2h20m — two independent defects (a duplicate PK inside one `ON CONFLICT DO UPDATE`, and FIFO charging the queue for a failing message's whole retry schedule).
8. **Prod down 8h31m** (10–11 September 2026): PlanetScale role deleted — prod's durable runtime credential was named `sydney-2026-07-14-depvs9`, indistinguishable from the short-lived temp roles every migration creates, and was swept up in a cleanup of exactly those. Availability plus a real ~8.5h ingest gap. The usher spool and the re-fetchable vendors recovered most of it; the live-poll-only vendors are permanently short. A **second, independent** fault: the `musher` collector died 4h into the outage and did not recover when the receiver returned, costing a further 4h52m at Daylesford while its sibling `fusher` looked perfectly healthy. A credential's NAME is a safety control.
