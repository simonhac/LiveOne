...
| Gap                       | Filled by                                                                                                                                                                                                                                                                                                                         |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ≤ ~45 min                 | normal rolling lookback — each poll re-requests the last **45 min** (`DEFAULT_LOOKBACK_MS`), so a just-published interval lands and recent revisions heal. Sized to outlast the `data` endpoint's publish lag (it trails `market`), so late power/emissions — and the derived `emissionsIntensity` — re-land before scrolling out |
| 45 min – **24 h**         | **adaptive lookback** — when behind (we/OE were down), the poll extends its window back to `lastSeenIntervalEndMs`, capped at `MAX_AUTOHEAL_MS` (24 h ≈ 288 intervals, one fetch). The next successful poll auto-fills the whole gap                                                                                              |
