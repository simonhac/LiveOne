...
-- This handles the production schema which has both old (solar_power) and new (solar_w) columns
...
SELECT 
  ...
  received_time, 
  delay_seconds,
  COALESCE(solar_w, ROUND(solar_power)) as solar_w,
  COALESCE(solar_inverter_w, ROUND(solar_inverter_power)) as solar_inverter_w,
  COALESCE(shunt_w, ROUND(shunt_power)) as shunt_w,
  COALESCE(load_w, ROUND(load_power)) as load_w,
  COALESCE(battery_w, ROUND(battery_power)) as battery_w,
  COALESCE(grid_w, ROUND(grid_power)) as grid_w,
  battery_soc,
