-- 0068 — CONTRACT. Drop `derivations.area_id` and `derivations.source_points`.
--
...
--
-- 🛑 **The `schema.ts` removal ships FIRST and is deployed BEFORE this runs.** Four reads of
-- `derivations` are whole-table projections (`.select({ d: derivations, … })` in
-- lib/derivations/scope.ts and resolve.ts, and a bare `.returning()` in v4-routes.ts), and drizzle
-- expands those to the DECLARED columns. A deployment still declaring a dropped column 42703s on
-- every read of the derivations surface. The same trap cost time at 0052 (`areas.legacy_system_id`
-- vs `prod-dev-sync`'s `neutralize`), so it is written down twice.
--
...
--     `ensureRunDetector`'s `owner-role-taken` check, which is CODE, not
--     a constraint, and honestly so: two detectors whose SIGNALS sit on different devices can still
--     resolve to the same OWNER device and fight over one `<stem>/running` point, and no index can
--     express that. See the note at lib/derivations/resolve.ts.
...
BEGIN
  ...
  IF unwired > 0 THEN
    RAISE EXCEPTION '0068: % derivation(s) have no required slot in derivation_sources — their wiring exists only in the source_points jsonb this migration drops: %', unwired, COALESCE(detail, 'NONE');
  END IF;
