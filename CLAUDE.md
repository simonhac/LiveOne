...
PLANETSCALE_PRODUCTION_HOST=<prod-unique token>                # arms the dev guardrail (assertNotProdDbInDev); matched against user@host. PlanetScale shares one regional host across branches, so set it to the prod BRANCH ID (in the prod username), not the hostname.
...
`liveone-dev` is a **separate** single-node PlanetScale database (`aws-ap-southeast-2`), the sole datastore for **both local dev and Vercel preview**. It is never prod: the app's `assertNotProdDbInDev` guardrail (armed by `PLANETSCALE_PRODUCTION_HOST` in dev/preview) refuses any connection whose identity carries the prod token. **Note:** PlanetScale puts every branch/database in a region on the **same gateway host** (e.g. `aws-ap-southeast-2-1.pg.psdb.cloud`) and distinguishes them by the role/username (`postgres.<branch-id>`) — so the guardrail token is the prod **branch id**, not the hostname, and `liveone-dev` is told apart from prod by its username. Routing is via env: prod connects through the discrete `DB_*` vars (Production scope); dev + preview set `PLANETSCALE_DATABASE_URL` to `liveone-dev` (it takes precedence over `DB_*` in `getPoolConfig`). Keep prod's `DB_*`/URL out of the Preview/Development scopes.
