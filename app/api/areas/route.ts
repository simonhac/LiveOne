...
/**
 * GET /api/areas?systemId=N — the P3 Area for a device, read-only.
 *
 * Returns the Area whose view is `systemId` — a single-device Area (a 1:1 wrapper over the physical
 * device) or a multi-device Area (points drawn across its `area_members`), plus the typed
 * `area_bindings`. A single-device Area carries NO bindings — its points are the device's own points
 * (resolved from `point_info`), so `bindings` is empty for it; multi-device Areas return their
 * role→point edges.
 *
 * Access is device-granular (`requireDeviceAccess`) — Areas are organisational, not the access
 * boundary, until P4. This is a NEW endpoint; the legacy `/api/data` payload stays frozen. Returns
 * `{ area: null, bindings: [] }` when no Area has been backfilled for the device yet, so the
...
export async function GET(request: NextRequest) {
  ...
  const resolved = await getAreaForDevice(systemId);
  if (!resolved) {
