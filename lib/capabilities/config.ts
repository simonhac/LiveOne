...
/**
 * Per-device battery-provenance knobs (off-grid generator source; the reserve-floor prior).
 *
 * 🛑 NO `exportTariff`, deliberately. The feed-in tariff used to live here as
 * `none | amber | schedule`, where `amber` carried no information at all — it resolved to the
 * `bidi.grid.export/rate` series the area already binds — and `schedule` was a second implementation
 * of "a price over time" in jsonb. An area's feed-in tariff IS its bound export-rate point; a site
 * whose tariff is not measured publishes one from a tariff device rather than describing it here.
 * "Do not credit exports" is said by not binding the rate. See `lib/battery-provenance/tariff.ts`.
 */
