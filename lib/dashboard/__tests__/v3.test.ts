...
describe("ensureSankeyCardIds", () => {
  ...
  it("preserves an existing sankey id (idempotent)", () => {
    ...
    const once = ensureSankeyCardIds(d);
    expect(once.sections[0].cards[0].id).toBe("keep");
    expect(ensureSankeyCardIds(once)).toEqual(once);
  });
