 ...
 * - `scanDocRefs` (here) walks RAW JSON and fails **open**. It answers "what would break if this
 *   went away", and for that question the closed reading is the wrong direction: a doc that fails
 *   the shape guard is not gone, it is BROKEN AND REPAIRABLE — its refs are still stored, still
 *   what a repair would restore, and still what a human describing the dashboard would name. Read
 *   through the typed walker such a doc references nothing at all, so deleting its area would look
 *   free. It also sees refs in positions the typed walker does not model, which is the same point:
 *   a stored doc can be older than the type.
 *
 *   ⚠️ Note what this does NOT claim. A doc failing `isDashboardV4` does not render its refs —
 *   `app/dashboard/[...slug]/page.tsx` substitutes an EMPTY document, deliberately, so a shape-guard
 *   failure cannot serve a second divergent shape. The argument for failing open is about repair,
 *   not about what is on screen today.
 *
