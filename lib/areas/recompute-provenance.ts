 ...
 * The two routes are therefore ONE function with two addresses; stage 13 deletes the legacy address and
...
/**
 * `recomputeAreaProvenance` — POST …/areas/{id}/recompute-provenance — materialise ONE Area's `point_readings_flow_attr_1d`
 ...
 */
export async function recomputeAreaProvenance(
  request: NextRequest,
  areaRef: string,
  ...
  const uuid = areaRefToUuid(areaRef);
  if (!uuid)
