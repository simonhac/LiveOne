 ...
 * /api/v4/areas/{ar_…}/recompute-provenance`) to completion. That endpoint materialises the attributed flow
...
export async function recomputeAreaFlow(
  ...
  for (let i = 0; i < 200; i++) {
    const res = await fetch(
      `/api/v4/areas/${encodeURIComponent(areaId)}/recompute-provenance`,
      {
        method: "POST",
