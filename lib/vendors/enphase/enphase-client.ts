 ...
 *
 * See docs/vendors/enphase-api.md for detailed API behavior documentation
 */
...
export class EnphaseClient implements IEnphaseClient {
  ...
  constructor() {
    ...
    if (!apiKey || !clientId || !clientSecret || !redirectUri) {
      ...
      console.error(
        "ENPHASE: Missing required environment variables:",
        missing.join(", "),
      );
      throw new Error(
        `Enphase configuration incomplete: Missing ${missing.join(", ")}`,
      );
    }
