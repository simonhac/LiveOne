...
| --- | --- | --- |
| ~~Operational and alert events~~ | ✅ **Implemented** (September 2026): `selectlive events info` / `events download`, `lib/selectlive/events.ts`, and the retained `device_events` table behind `liveone device events`. Both log layouts, the 36-word record, the code tables and the scaling block are traced and live-verified; unknown codes and raw records are preserved. Retention measured on firmware 12.25: 52 alerts over 15 months, 910 operational records over one week. | — |
