 ...
 *
 * Self-contained (depends only on `modbus-serial`) so it can later graduate into
 * `lib/vendors/deepsea/dse-client.ts`, exactly as `scripts/sigenergy/sigen-client.ts`
 * graduated into `lib/vendors/sigenergy/sigenergy-client.ts`.
 *
 * GenComm specifics:
