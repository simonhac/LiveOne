...
describe("check-readings-boundary", () => {
  ...
  describe("false-positive avoidance (temp fixtures, empty baseline)", () => {
    ...
    beforeAll(() => {
      ...
      // The lookahead must reject a longer word that merely starts with the table name, the same way it
      // rejects `_flow_attr_1d`. (This also covered the cutover-era `_old` suffix group, removed once
      // migrations 0038/0039 retired the `_old`/`_new` spellings.)
      writeFileSync(
