...
const OWNERSHIP: ReadonlyArray<{ table: string; col: string; where?: string }> =
  [
    ...
    // Legacy per-device dashboards (and their `system_id` column) were dropped by migration 0022 —
    // every remaining row is a composition dashboard, so no filter is needed.
