...
import { systems, enphaseCredentials } from "@/lib/db/turso/schema";
...
async function fetchRawEnphaseData() {
  ...
  const [startUnix, endUnix] = calendarDateToUnixRange(
    date,
    system.timezoneOffsetMin,
  );

  console.log(
    `Date range: ${new Date(startUnix * 1000).toISOString()} to ${new Date(endUnix * 1000).toISOString()}`,
  );

  // Check if we're in development or production
  const isDev =
    process.env.NODE_ENV === "development" || !process.env.TURSO_DATABASE_URL;
  ...
  if (isDev) {
    ...
    if (data.intervals && data.intervals.length > 0) {
      const totalEnergy = data.intervals.reduce(
        (sum: number, i: any) => sum + (i.enwh || 0),
        0,
      );
      ...
      console.log(
        `- First interval: ${new Date(data.intervals[0].end_at * 1000).toISOString()}`,
      );
      console.log(
        `- Last interval: ${new Date(data.intervals[data.intervals.length - 1].end_at * 1000).toISOString()}`,
      );
    }
  } else {
    ...
    if (!credentials) {
      console.error(
        `No Enphase credentials found for user ${system.ownerClerkUserId}`,
      );
      process.exit(1);
    }
