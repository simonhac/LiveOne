#!/usr/bin/env npx tsx
/**
 * Fetch last 7 days of Enphase data for a device
...
async function fetchEnphaseWeek() {
  ...
  if (!params.systemId || !params.environment) {
    ...
    console.error(
      "Example: ./scripts/utils/fetch-enphase-week.ts --systemId=3 --environment=dev",
    );
    console.error(
      "Example: ./scripts/utils/fetch-enphase-week.ts --systemId=3 --environment=prod --cronSecret=<secret>",
    );
    process.exit(1);
  }
  ...
  if (environment === "prod" && !cronSecret) {
    ...
    console.error(
      "2. Set it as an environment variable: CRON_SECRET=<secret> ./scripts/utils/fetch-enphase-week.ts ...",
    );
    process.exit(1);
  }
  ...
  console.log(
    `Fetching last 7 days of data for system ${systemId} (${environment})...`,
  );
    ...
    try {
      ...
      if (data.success && data.results?.[0]) {
        ...
        if (result.status === "polled") {
          ...
          successCount++;
          console.log(
            `  ✓ Success: ${recordsUpserted} records upserted (${result.durationMs}ms)`,
          );
  ...
  console.log(
    `Average per day: ${Math.round(totalRecords / (successCount || 1))} records`,
  );
}
