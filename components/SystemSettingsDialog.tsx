...
interface SystemSettingsDialogProps {
  ...
  onUpdate: (systemId: number, updates: { displayName?: string, shortName?: string | null }) => Promise<void>
}
...
export default function SystemSettingsDialog({
  ...
  const [editedShortName, setEditedShortName] = useState(system?.shortName || '')
  ...
  const handleSaveShortName = async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('UNIQUE constraint failed')) {
        setShortNameError(`Short name "${editedShortName}" is already in use for ${system.vendorType} systems`)
      }
    } finally {
      setIsSaving(false)
    }
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mb-2">
                Used in history API IDs. Only letters, digits, and underscores. Must be unique for {system?.vendorType} systems.
              </p>
              <div className="flex items-center gap-2">
                <input
                  type="text"
