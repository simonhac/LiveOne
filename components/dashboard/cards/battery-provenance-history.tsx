 ...
 * public area id); on a real area dashboard the section's areaId is used as-is (already `ar_`, the
 * read-normalized descriptor). Both are normalized to `ar_` before reaching the panel, so no raw
 * uuid reaches the `/api/areas/*` wire.
 *
 ...
 * Area; that page now renders a DEVICE-bound v4 document with no area envelope at all, so the adapter
 * hands this card `section.areaId === ""`. `areaRefToArId("")` is null — the same "no area" outcome
 * the sentinel branch produced — so the removal is behaviour-preserving as well as producer-less.
 * (Independently: every device on both environments now has an area-of-one, so the sentinel had
 * already stopped being reachable in practice.)
 */
